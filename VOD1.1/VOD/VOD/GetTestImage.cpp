...
std::vector<std::vector<char>> GetTestImage::get_ImageFingerprint(std::string image_path)
{
	Mat src, result;
	...
	try {
		...
		v = s->GenerateServerSample(test_Image, test_Image->width / 2, test_Image->height / 2, test_Image->width / 2, test_Image->height / 2);
